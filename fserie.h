...
bool seriewritestrn(char *sdata, int nb);
void closeserie();
...
bool initseriecc(char *serial_name, long baud)
{
  ...
  switch(baud)
  {
   ...
   case 115200:baudx=B115200;
              break;
   case 500000:baudx=B500000;
              break;

   case 1000000:baudx=B1000000;
                 break;

   case 2000000:baudx=B2000000;
                 break;
...
bool seriewritestrn(char *sdata,int nb)
{
 ...
 bool resu;
  ...
  if( hport != INVALID_HANDLE_VALUE)
	   {
	WriteFile(hport,sdata,nb , &dwCharsWritten, NULL);
	if( dwCharsWritten == nb)
		 resu = true;
	}
